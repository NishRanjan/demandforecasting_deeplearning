"""
models/nhits_model.py
─────────────────────
N-HiTS (Neural Hierarchical Interpolation for Time Series) forecaster.

N-HiTS is available in pytorch-forecasting and follows the same
TimeSeriesDataSet + Lightning Trainer pattern as TFT.
It is lighter than TFT (no attention, no static categoricals natively)
but often competitive on univariate and low-feature scenarios.
"""
...
class NHiTSForecaster(BaseForecaster):
    ...
    def fit(self, train_df: pd.DataFrame) -> None:
        """Build TimeSeriesDataSet and train NHiTS with Lightning."""
        try:
            from pytorch_forecasting import TimeSeriesDataSet, NHiTS
            from pytorch_forecasting.data import GroupNormalizer
            from pytorch_forecasting.metrics import MASELoss
        ...
        # NHiTS works well with a lean feature set (target + time index)
        ...
        print(f"[nhits] Training NHiTS: encoder={max_encoder_length}, horizon={max_prediction_length}")
        trainer.fit(self._model, train_dataloaders=train_loader, val_dataloaders=val_loader)
        self._min_date = min_date
        self._group_ids = group_ids
        self._df_fit = df
        self._is_fitted = True
        print("[nhits] Training complete.")

    def predict(self, context_df: pd.DataFrame, horizon: int) -> pd.DataFrame:
        """Generate forecasts from the most recent context."""
        self._check_fitted()
    ...
    def backtest(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"[nhits] Backtest cutoffs: {[str(c.date()) for c in cutoffs]}")
